...
<main>
  ...
  <header class="h-screen -mt-20 flex align-middle justify-center items-center bg-bg-400">
    <div use:reveal={{ transition: "slide", x: -80, easing: 'easeOutBack' }}>
  ...
  <section class="px-[5%] py-12 md:px-[10%] md:py-20">
    <div class="bg-bg-500 p-8 md:p-16 rounded-2xl md:mx-[7%]" use:reveal={{ transition: "slide", x: -80, easing: 'easeOutBack' }}>
    ...
    <div class="bg-bg-500 p-8 md:p-16 rounded-2xl md:mx-[7%]" use:reveal={{ transition: "slide", x: -80, easing: 'easeOutBack' }}>
      ...
      <div class="flex flex-row flex-wrap gap-2">
        {#each skills as skill, idx}
          <div class="bg-blue/5 text-blue px-3 py-1.5 rounded-lg font-semibold text-sm duration-100 hover:bg-blue/10" use:reveal={{ transition: "spin fly", rotate: -10, y:-20, duration: 500, delay: 75*idx }}>
            {skill}
          </div>
        {/each}
      </div>
  ...
  <section class="px-[5%] pt-6 pb-16 md:px-[10%]" use:reveal={{ transition: "fly", y: 50 }}>
    <h1 class="text-2xl text-center font-semibold">Get in touch</h1>
    <p class="text-center">
    ...
    <div class="mx-auto w-10/12 md:w-2/3 mt-8">
      <form action=”mailto:mehedirm6244@protonmail.com” autocomplete="off" bind:this={contactForm} on:submit|preventDefault={sendMail}>
        ...
        <input name="title" placeholder="Title" class="block w-full bg-bg-500 px-6 py-4 rounded-xl my-4">

        <p class="text-xl font-medium mb-2">Content</p>
        <textarea name="content" placeholder="Content" rows="4" class="block w-full resize-none bg-bg-500 px-6 py-4 rounded-xl my-4"></textarea>
